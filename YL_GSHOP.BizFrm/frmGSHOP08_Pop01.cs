...
{
    public partial class frmGSHOP08_Pop01 : FrmPopUpBase
    {
        ...
        public frmGSHOP08_Pop01()
        {
            InitializeComponent();
        }

        private void FrmGSHOP08_Pop01_Load(object sender, EventArgs e)
        {
            this.Text = "상세정보";
        ...
        private void Btnchg_Click(object sender, EventArgs e)
        {
            if (MessageAgent.MessageShow(MessageType.Confirm, "노출상태를 변경 하시겠습니까?") == DialogResult.OK)
            {
                try
                {
                    using (MySqlConnection con = new MySqlConnection(ConstantLib.BasicConn_Real))
                    {
                        using (MySqlCommand cmd = new MySqlCommand("domabiz.USP_GSHOP08_SAVE_10", con))
                        {
                            con.Open();
                            cmd.CommandType = CommandType.StoredProcedure;
